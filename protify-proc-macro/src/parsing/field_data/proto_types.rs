...
impl ProtoType {
			...
			"enum_" => {
				let path = if let Ok(path) = meta.parse_list::<Path>() {
					...
					type_info
						...
						.ok_or_else(|| {
							meta.error("Failed to infer the enum path. Please set it manually")
						})?
				};
	...
	pub fn validator_name(&self, span: Span) -> TokenStream2 {
		match self {
			...
			Self::Sfixed32 => {
				quote_spanned! {span=> ::protify::IntValidator<::protify::Sfixed32>  }
			}
			Self::Sfixed64 => {
				quote_spanned! {span=> ::protify::IntValidator<::protify::Sfixed64>  }
			}
	...
	pub fn output_proto_type(&self, span: Span) -> TokenStream2 {
		match self {
			...
			Self::Bool => quote_spanned! {span=> bool },
			Self::Bytes => quote_spanned! {span=> ::protify::Bytes },
			Self::Enum(_) | Self::Int32 | Self::Sint32 | Self::Sfixed32 => {
				quote_spanned! {span=> i32 }
			}
